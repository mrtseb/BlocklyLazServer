...
class Adafruit_Fingerprint : public virtualGroveConnector {
 public:

#if ARDUINO >= 100
  Adafruit_Fingerprint(SoftwareSerial *, int _setup=false);
#else
  ...
  boolean enrollFingerPrintPass1(uint8_t id);
  boolean enrollFingerPrintPass2(uint8_t id);
  boolean getFingerprintIDez();
  boolean fingerAvailable();
  uint16_t getIDnumber()   { return fingerID;}
  uint16_t getIDnumber2();//   { return fingerID;}
  uint16_t getConfidence() { return confidence;}
  boolean isAvailable() { return _isAvailable;}
