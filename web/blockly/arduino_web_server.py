...
class Handler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if length:
            ...
            dirname = "D:/mrt/xampp/htdocs/blockly"
            sketchname = os.path.join(dirname, "default") + ".ino"
            f = open(sketchname, "wb")
            f.write(text + "\n")
            writefile(text,False)
            f.close()
            print ("created sketch at %s" % (sketchname,))
            writefile("created sketch at %s" % (sketchname,));
            ...
            writefile("Uploading with %s" % (" ".join(compile_args)));
            writefile("Patience ...");
            ...
            if not rc == 0:
                ...
                writefile("arduino --upload returned " + `rc`)				
